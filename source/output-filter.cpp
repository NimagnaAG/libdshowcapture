...
OutputPin::OutputPin(OutputFilter *filter_, VideoFormat format, int cx, int cy, long long interval)
    : OutputPin(filter_) {
  curCX = cx;
  curCY = cy;
...
STDMETHODIMP OutputPin::Connect(IPin *pReceivePin, const AM_MEDIA_TYPE *pmt) {
  ...
  if (filter->state == State_Running) return VFW_E_NOT_STOPPED;

  if (connectedPin) return VFW_E_ALREADY_CONNECTED;
...
STDMETHODIMP OutputPin::Disconnect() {
  ...
  if (!connectedPin) return S_FALSE;
...
STDMETHODIMP OutputPin::ConnectionMediaType(AM_MEDIA_TYPE *pmt) {
  ...
  if (!connectedPin) return VFW_E_NOT_CONNECTED;
...
STDMETHODIMP OutputPin::SetFormat(AM_MEDIA_TYPE *pmt) {
  ...
  if (pmt == nullptr) return VFW_E_INVALIDMEDIATYPE;
...
STDMETHODIMP OutputPin::Get(REFGUID guidPropSet, DWORD dwPropID, void *, DWORD, void *pPropData,
...
bool OutputPin::AllocateBuffers(IPin *target, bool connecting) {
  ...
  if (hr == VFW_E_NO_ALLOCATOR)
    hr = CoCreateInstance(CLSID_MemoryAllocator, NULL, CLSCTX_INPROC_SERVER,
                          __uuidof(IMemAllocator), (void **)&allocator);
...
bool OutputPin::LockSampleData(unsigned char **ptr) {
  if (!connectedPin) return false;

  ComQIPtr<IMemInputPin> memInput(connectedPin);
  HRESULT hr;

  if (!memInput || !allocator) return false;

  hr = allocator->GetBuffer(&sample, nullptr, nullptr, 0);
  if (FAILED(hr)) return false;

  if (FAILED(sample->SetActualDataLength((long)bufSize))) return false;
  if (FAILED(sample->SetDiscontinuity(false))) return false;
  if (FAILED(sample->SetPreroll(false))) return false;
  if (FAILED(sample->GetPointer(ptr))) return false;
...
void OutputPin::Send(unsigned char *data[DSHOW_MAX_PLANES], size_t linesize[DSHOW_MAX_PLANES],
                     long long timestampStart, long long timestampEnd) {
  BYTE *ptr;
  if (!LockSampleData(&ptr)) return;
...
STDMETHODIMP OutputFilter::FindPin(LPCWSTR Id, IPin **ppPin) {
  ...
  if (Id == nullptr || ppPin == nullptr) return E_POINTER;
...
// IEnumMediaTypes
STDMETHODIMP OutputEnumMediaTypes::Next(ULONG cMediaTypes, AM_MEDIA_TYPE **ppMediaTypes,
                                        ULONG *pcFetched) {
