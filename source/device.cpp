...
inline void HDevice::SendToCallback(bool video, unsigned char *data, size_t size,
                                    long long startTime, long long stopTime, long rotation) {
  if (!size) return;

  if (video)
    videoConfig.callback(videoConfig, data, size, startTime, stopTime, rotation);
  else
...
void HDevice::Receive(bool isVideo, IMediaSample *sample) {
  ...
  if (isVideo ? !videoConfig.callback : !audioConfig.callback) return;
  ...
  if (FAILED(sample->GetPointer(&ptr))) return;
  ...
  if (encoded) {
     ...
     * segments */
    if (hasTime) {
      SendToCallback(isVideo, data.bytes.data(), data.bytes.size(), data.lastStartTime,
...
void HDevice::ConvertVideoSettings() {
  ...
  if (bmih) {
    ...
    if (same) videoConfig.format = videoConfig.internalFormat;
  }
}

void HDevice::ConvertAudioSettings() {
  WAVEFORMATEX *wfex = reinterpret_cast<WAVEFORMATEX *>(audioMediaType->pbFormat);
...
bool HDevice::SetupExceptionVideoCapture(IBaseFilter *filter, VideoConfig &config) {
  ComPtr<IPin> pin;
...
bool HDevice::SetupVideoCapture(IBaseFilter *filter, VideoConfig &config) {
  ...
  rotatableDevice = videoConfig.name.find(L"StreamCam") != std::string::npos;
...
bool HDevice::SetVideoConfig(VideoConfig *config) {
  ...
  if (!EnsureInitialized(L"SetVideoConfig") || !EnsureInactive(L"SetVideoConfig")) return false;
  ...
  bool success = GetDeviceFilter(CLSID_VideoInputDeviceCategory, config->name.c_str(),
                                 config->path.c_str(), &filter);
  if (!success) {
    Error(L"Video device '%s': %s not found", config->name.c_str(), config->path.c_str());
    return false;
  }
  ...
  if (!SetupVideoCapture(filter, videoConfig)) return false;
...
bool HDevice::SetupExceptionAudioCapture(IPin *pin) {
  ...
  if (FAILED(hr)) {
    WarningHR(
        L"SetupExceptionAudioCapture: pin->EnumMediaTypes "
        L"failed",
        hr);
    return false;
  }
  ...
  if (enumMediaTypes->Next(1, &mt, &count) == S_OK && mt->formattype == FORMAT_WaveFormatEx) {
    audioMediaType = mt;
    return true;
  }
...
bool HDevice::SetupAudioCapture(IBaseFilter *filter, AudioConfig &config) {
  ...
  success = GetFilterPin(filter, MEDIATYPE_Audio, PIN_CATEGORY_CAPTURE, PINDIR_OUTPUT, &pin);
  if (!success) {
  ...
  if (config.useDefaultConfig) {
    ...
    if (pinConfig && SUCCEEDED(pinConfig->GetFormat(&defaultMT))) {
      if (is24BitAudio(defaultMT)) {
        WAVEFORMATEX *wfex = (WAVEFORMATEX *)defaultMT->pbFormat;
      ...
      if (!SetupExceptionAudioCapture(pin)) {
        Error(
            L"Could not get default format for "
            L"audio pin");
        return false;
      }
    }
  ...
  if (!config.useVideoDevice) graph->AddFilter(audioFilter, L"Audio Filter");
  return true;
}
...
bool HDevice::SetAudioConfig(AudioConfig *config) {
  ...
  if (!EnsureInitialized(L"SetAudioConfig") || !EnsureInactive(L"SetAudioConfig")) return false;

  if (!audioConfig.useVideoDevice) graph->RemoveFilter(audioFilter);
  ...
  if (!config->useVideoDevice && !config->useSeparateAudioFilter && config->name.empty() &&
      config->path.empty()) {
  ...
  if (config->useVideoDevice) {
    if (videoFilter == NULL) {
      Error(
          L"Tried to use video device's built-in audio, "
    ...
    if (!success) {
      Error(L"Corresponding audio device for '%s' not found", videoConfig.path.c_str());
      return false;
    }

  } else {
    bool success = GetDeviceFilter(CLSID_AudioInputDeviceCategory, config->name.c_str(),
                                   config->path.c_str(), &filter);
    if (!success) {
      Error(L"Audio device '%s': %s not found", config->name.c_str(), config->path.c_str());
      return false;
    }
  }
  ...
  if (config->mode == AudioMode::Capture) {
    if (!SetupAudioCapture(filter, audioConfig)) return false;
...
bool HDevice::CreateGraph() {
  ...
  if (!CreateFilterGraph(&graph, &builder, &control)) return false;
...
bool HDevice::FindCrossbar(IBaseFilter *filter, IBaseFilter **crossbar) {
  ...
  hr = builder->FindInterface(NULL, NULL, filter, IID_IAMCrossbar, (void **)crossbar);
  if (SUCCEEDED(hr)) return true;

  if (!GetPinByName(filter, PINDIR_INPUT, nullptr, &pin)) return false;
  if (!GetPinMedium(pin, medium)) return false;
  if (!GetFilterByMedium(AM_KSCATEGORY_CROSSBAR, medium, crossbar)) return false;

  graph->AddFilter(*crossbar, L"Crossbar Filter");
  return true;
}

bool HDevice::ConnectPins(const GUID &category, const GUID &type, IBaseFilter *filter,
                          IBaseFilter *capture) {
  HRESULT hr;
  ...
  if (!EnsureInitialized(L"HDevice::ConnectPins") || !EnsureInactive(L"HDevice::ConnectPins"))
    return false;

  if (connectCrossbar && FindCrossbar(filter, &crossbar)) {
    if (!DirectConnectFilters(graph, crossbar, filter)) {
      Warning(
          L"HDevice::ConnectPins: Failed to connect "
          L"crossbar");
      return false;
    }
  }
...
bool HDevice::RenderFilters(const GUID &category, const GUID &type, IBaseFilter *filter,
                            IBaseFilter *capture) {
  HRESULT hr;

  if (!EnsureInitialized(L"HDevice::RenderFilters") || !EnsureInactive(L"HDevice::RenderFilters"))
    return false;
...
void HDevice::SetAudioBuffering(int bufferingMs) {
  ComPtr<IPin> pin;
  bool success =
      GetFilterPin(audioFilter, MEDIATYPE_Audio, PIN_CATEGORY_CAPTURE, PINDIR_OUTPUT, &pin);
  if (!success) return;
  ...
  MediaTypePtr mt;
  if (FAILED(config->GetFormat(&mt))) return;

  if (mt->formattype != FORMAT_WaveFormatEx) return;
  if (mt->cbFormat != sizeof(WAVEFORMATEX)) return;
  ...
  if (FAILED(hr))
    WarningHR(
        L"Could not set allocator properties on audio "
        L"capture pin",
        hr);
}
...
bool HDevice::ConnectFilters() {
  ...
  if (!EnsureInitialized(L"ConnectFilters") || !EnsureInactive(L"ConnectFilters")) return false;

  if (videoCapture != NULL) {
    /* use hardware tonemapper for narrow format (SDR), not wide (HDR) */
    const bool enable_tonemapper = videoConfig.format != VideoFormat::P010;
    SetVendorTonemapperUsage(videoFilter, enable_tonemapper);

    success = ConnectPins(PIN_CATEGORY_CAPTURE, MEDIATYPE_Video, videoFilter, videoCapture);
    if (!success) {
      success = RenderFilters(PIN_CATEGORY_CAPTURE, MEDIATYPE_Video, videoFilter, videoCapture);
    }
  }

  if ((audioCapture || audioOutput) && success) {
    IBaseFilter *filter = (audioCapture != nullptr) ? audioCapture.Get() : audioOutput.Get();
    ...
    bool streamEngine = audioConfig.useVideoDevice &&
                        (videoConfig.name.find(L"Stream Engine") != std::string::npos);

    if (!streamEngine && audioCapture != nullptr) SetAudioBuffering(10);

    success = ConnectPins(PIN_CATEGORY_CAPTURE, MEDIATYPE_Audio, audioFilter, filter);
    if (!success) {
      success = RenderFilters(PIN_CATEGORY_CAPTURE, MEDIATYPE_Audio, audioFilter, filter);
    }
  }

  if (success) LogFilters(graph);
...
Result HDevice::Start() {
  ...
  if (!EnsureInitialized(L"Start") || !EnsureInactive(L"Start")) return Result::Error;

  if (!!rocketEncoder) Sleep(ROCKET_WAIT_TIME_MS);
